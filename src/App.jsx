...
export default function App() {
  ...
  const removeAbs = async (eid, day, weekKey, knownType) => {
    ...
    try {
      await txSchedule(({ entries: s, absences: a }) => {
        // Půlden: člověk ve směně zůstal → stačí sundat označení, umístění (i ruční) se zachová
        if (!isFullAbs(a[k] ?? prevType)) {
          SHIFTS.forEach(sh => (s[day]?.[sh] || []).forEach(e => { if (e.empId === eid) { delete e.halfAbs; delete e.halfPart; } }));
      ...
      <main style={{ padding: 16, maxWidth: 1000, margin: "0 auto" }}>
        <div key={viewKey.current} className="avi">
          ...
          {view === "schedule" && <ScheduleView
            analysis={analysis} cs={cs} cw={cw} dayHol={dayHol} intake={intake} intakeAllow={intakeAllow} isA={isA} isMobile={isMobile} notes={notes} profile={profile} rules={rules} schedMeta={schedMeta} schedView={schedView} selDay={selDay} slideDir={slideDir} wd={wd} wh={wh} wo={wo}
            allowIntakeException={allowIntakeException} canDrag={canDrag} exportCSV={exportCSV} ge={ge} getDayAbs={getDayAbs} getEntries={getEntries} goDay={goDay} handleDrop={handleDrop} removeAbs={removeAbs} setModal={setModal} setNoteView={setNoteView} setSchedView={setSchedView} setSelCell={setSelCell} setShowCompare={setShowCompare} setShowPerma={setShowPerma} setWo={setWo} switchV={switchV} toggleIntake={toggleIntake} />}

          {/* ═══ OTHER VIEWS ═══ */}
          {view === "proposals" && <ProposalsView isA={isA} profile={profile} yearProblems={yearProblems} visibleProps={visibleProps} myPendingProps={myPendingProps} ge={ge}
            onApplyFix={applyProblemFix} onConsent={consentProposal} onReject={rejectProposal} />}

          {view === "swaps" && <SwapsView isA={isA} profile={profile} swaps={openSw} ge={ge}
