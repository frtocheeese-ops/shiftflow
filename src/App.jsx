...
const ABS = [
  ...
  { id: "half_vacation", label: "½ Dovolená", icon: "🏖️", color: "#4080b0" },
...
function fmtDate(isoStr) { const d = new Date(isoStr); return `${d.getDate()}.${d.getMonth() + 1}.`; }
const todayIdx = (() => { const d = new Date().getDay(); return d >= 1 && d <= 5 ? d - 1 : 0; })();
const isToday = (i, wo) => wo === 0 && i === todayIdx;
...
:root,[data-theme="light"]{
  ...
  --brd:rgba(50,60,85,.22);--brd2:rgba(40,50,75,.35);--brd-t:rgba(35,45,70,.45);
  --tx:#2e3440;--tx2:#555e70;--tx3:#7a8290;--w:#1a1e28;
  --acc:#3a4558;--acc2:#d47820;--acc3:#c05828;--acc-dim:rgba(212,120,32,.1);--acc-brd:rgba(212,120,32,.32);
  --l1:#5050d0;--sd:#2878a8;--red:#b83030;--grn:#388040;--amb:#c87020;
  --sel:#2e3848;--sel-tx:#e8eaf0;--blur:blur(24px);--glass:rgba(215,220,230,.65);
...
[data-theme="dark"]{
  ...
  --brd:rgba(255,255,255,.08);--brd2:rgba(255,255,255,.12);--brd-t:rgba(255,255,255,.14);
  --tx:#c0c4d0;--tx2:#7880a0;--tx3:#4a5070;--w:#e8eaf0;
  --acc:#7b8fad;--acc2:#d47820;--acc3:#c05828;--acc-dim:rgba(212,120,32,.12);--acc-brd:rgba(212,120,32,.3);
  --l1:#7c7cf5;--sd:#50a0d0;--red:#c04040;--grn:#50a060;--amb:#c87020;
  --sel:rgba(123,143,173,.15);--sel-tx:#e8eaf0;--blur:blur(16px);--glass:rgba(16,16,26,.82);
...
@keyframes todayP{0%,100%{box-shadow:0 0 0 0 rgba(212,120,32,.4)}50%{box-shadow:0 0 0 4px rgba(212,120,32,0)}}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}
@keyframes slideToast{from{transform:translateY(-30px);opacity:0}to{transform:translateY(0);opacity:1}}
.vi{animation:viewIn .35s cubic-bezier(.22,.68,.36,1)}.sr{animation:slideR .26s both}.sl{animation:slideL .26s both}
.tp{animation:todayP 2.5s ease-in-out infinite}
...
.glass{background:var(--card);backdrop-filter:var(--blur);-webkit-backdrop-filter:var(--blur);border:1px solid var(--brd-t);position:relative;overflow:hidden}
...
function ParallaxBg() {
  ...
  useEffect(() => {
    ...
    const upd = (x, y) => { if (el) el.style.transform = `translate(${x * 24}px,${y * 18}px) scale(1.08)`; };
  ...
  return <div ref={ref} style={{ position: 'fixed', inset: '-60px', zIndex: 0, pointerEvents: 'none', backgroundImage: 'var(--moon)', backgroundSize: 'cover', willChange: 'transform', transition: 'transform .1s linear' }} />;
}

/* ═══ NAVIGATION ═══ */
function SideNav({ view, setView, NAV }) {
  return <aside className="pg" style={{ width: 200, flexShrink: 0, borderRight: "1px solid var(--brd-t)", display: "flex", flexDirection: "column", gap: 2, padding: "20px 0" }}>
    <div style={{ padding: "0 20px 20px", fontSize: 22, fontWeight: 700, color: "var(--w)", letterSpacing: 4, fontFamily: "'Barlow Condensed',sans-serif", borderBottom: "1px solid var(--brd)" }}>SHIFTFLOW</div>
    <div style={{ padding: "12px 0", flex: 1 }}>
      {NAV.map(n => <button key={n.id} onClick={() => setView(n.id)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "12px 20px", border: "none", background: view === n.id ? "var(--acc-dim)" : "transparent", borderLeft: view === n.id ? "3px solid var(--acc2)" : "3px solid transparent", color: view === n.id ? "var(--acc2)" : "var(--tx2)", cursor: "pointer", fontSize: 14, fontWeight: 600, fontFamily: "'Barlow Condensed',sans-serif", textTransform: "uppercase", letterSpacing: 1, transition: "all .2s", minHeight: 44, textAlign: "left", position: "relative" }}>
...
function PillNav({ view, setView, NAV }) {
  return <nav className="pg" style={{ position: 'fixed', bottom: 16, left: '50%', transform: 'translateX(-50%)', zIndex: 100, display: 'flex', gap: 2, padding: 4, border: '1px solid var(--brd-t)', boxShadow: '0 8px 32px rgba(0,0,0,.22)' }}>
    {NAV.map(n => <button key={n.id} onClick={() => setView(n.id)} style={{ display: 'flex', alignItems: 'center', gap: view === n.id ? 7 : 0, padding: view === n.id ? '10px 16px' : '10px 13px', border: 'none', background: view === n.id ? 'var(--acc-dim)' : 'transparent', outline: view === n.id ? '1px solid var(--acc-brd)' : 'none', cursor: 'pointer', color: view === n.id ? 'var(--acc2)' : 'var(--tx3)', transition: 'all .28s cubic-bezier(.22,.68,.36,1)', minHeight: 44, position: 'relative' }}>
      <span style={{ fontSize: 18 }}>{n.i}</span>
      <span style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1.2, overflow: 'hidden', maxWidth: view === n.id ? 80 : 0, whiteSpace: 'nowrap', transition: 'max-width .28s' }}>{n.l}</span>
...
function Setup({ profile, onDone }) {
  ...
  return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", padding: 16 }}><style>{CSS}</style><ParallaxBg />
    <div className="glass" style={{ width: "100%", maxWidth: 520, padding: "36px 24px", animation: "modalUp .5s", position: "relative", zIndex: 1 }}>
      ...
      <Btn warm disabled={saving} onClick={async () => { setSaving(true); await updateDoc(doc(db, "users", profile.id), { defaultSchedule: sched, setupDone: true }); onDone(); }} style={{ width: "100%", marginTop: 24, padding: "14px 0", fontSize: 17 }}>{saving ? "UKLÁDÁM..." : "POTVRDIT"}</Btn>
    </div>
  </div>;
}
...
function AuthScreen() {
  ...
  const doBio = async () => { try { const s = localStorage.getItem("sf_bio_email"), p = localStorage.getItem("sf_bio_token"); if (!s || !p) return setErr("Přihlaste se heslem a povolte biometrii"); await signInWithEmailAndPassword(auth, s, p); } catch (e) { setErr("Biometrie: " + e.message); } };
  return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", padding: 16 }}><style>{CSS}</style><ParallaxBg />
    <div className="glass" style={{ width: "100%", maxWidth: 440, padding: "40px 28px", animation: "modalUp .5s", position: "relative", zIndex: 1 }}>
      ...
      {mode === "login" ? <>
        ...
        <div style={{ display: "flex", gap: 8 }}><Btn warm disabled={loading} onClick={doLogin} style={{ flex: 1 }}>{loading ? "..." : "Přihlásit"}</Btn><Btn ghost onClick={doBio} style={{ fontSize: 20 }}>🔐</Btn></div>
      </> : <>
        ...
        <Input label="Heslo" type="password" value={rp} onChange={e => setRp(e.target.value)} />
  ...
  return <div>{["L1", "SD"].map(team => <div key={team} style={{ marginBottom: 28 }}>
    ...
    <Modal open={!!editEmp} onClose={() => setEditEmp(null)} title={editEmp?.name || ""}>{editEmp && <div>
      {DAYS.map(day => <div key={day} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "var(--bg3)", marginBottom: 4 }}>
        ...
        <div style={{ display: "flex", gap: 2, flex: 1 }}>{SHIFTS.map(sh => <button key={sh} onClick={() => setEs(s => ({ ...s, [day]: sh }))} style={{ flex: 1, padding: "10px 0", border: `1px solid ${es[day] === sh ? "var(--acc2)" : "var(--brd)"}`, fontFamily: "'IBM Plex Mono',monospace", cursor: "pointer", background: es[day] === sh ? "var(--acc-dim)" : "transparent", color: es[day] === sh ? "var(--w)" : "var(--tx3)", minHeight: 44 }}>{sh}</button>)}</div>
        <label style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 13, color: "var(--tx3)" }}><input type="checkbox" checked={es[`${day}_ho`] || false} onChange={e => setEs(s => ({ ...s, [`${day}_ho`]: e.target.checked }))} style={{ accentColor: "var(--grn)", width: 18, height: 18 }} />HO</label>
      </div>)}
...
export default function App() {
  ...
  // Absence: optimistic, handles HO conflict by removing entry first
  const addAbs = async (eid, day, type) => {
    if (!isA && eid !== profile.id) return;
    const s = dc(cs);
    // Remove employee from ALL shifts on that day (including HO)
    SHIFTS.forEach(sh => { if (s[day]?.[sh]) s[day][sh] = s[day][sh].filter(e => e.empId !== eid); });
    setSchedule(s); setAbsences(prev => ({ ...prev, [`${eid}-${day}`]: type }));
    try {
      await setDoc(doc(db, "schedules", wk), { entries: s, [`absences.${eid}-${day}`]: type, weekStart: wk, modifiedAt: new Date().toISOString() }, { merge: true });
      const emp = ge(eid);
      ...
      notify(`${emp?.name}: ${ABS.find(a => a.id === type)?.label}`); log(`${emp?.name}: ${ABS.find(a => a.id === type)?.label} ${day}`);
    } catch (err) { console.error(err); notify('Chyba'); }
  };

  const saveNote = async (eid, day, shift, note) => {
    const key = `${eid}-${day}-${shift}`;
    setNotes(p => ({ ...p, [key]: note }));
    await setDoc(doc(db, "schedules", wk), { [`notes.${key}`]: note }, { merge: true });
  ...
  const doSwap = async (swId, aid) => {
    ...
    setSchedule(s);
    try { await updateDoc(doc(db, 'swapRequests', swId), { status: 'done', aid, resolvedAt: new Date().toISOString() }); await saveS(s); notify('Výměna provedena'); } catch { notify('Chyba'); }
  };
  ...
  // Employee can D&D within their team
  const canDrag = (empId) => isA || empId === profile?.id;
  const canDrop = (empId, targetDay) => {
    if (isA) return true;
    const emp = ge(empId); const me = profile;
    return empId === me?.id && emp?.team === me?.team;
  };
  ...
  const dayEntries = (shift) => (cs[DAYS[selDay]]?.[shift] || []).filter(e => { const emp = ge(e.empId); return emp && (tf === "all" || emp.team === tf); });
  const dayAbs = Object.entries(absences).filter(([k]) => k.endsWith(`-${DAYS[selDay]}`)).map(([k, t]) => ({ empId: k.replace(`-${DAYS[selDay]}`, ""), type: t })).filter(a => ge(a.empId));
  ...
  return <div style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "'Barlow',sans-serif", color: "var(--tx)", display: "flex" }} data-theme={theme}>
    ...
    {/* TOASTS */}
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 9999, padding: "8px 12px" }}>{notifs.map(n => <div key={n.id} className="glass" style={{ padding: "14px 16px", fontSize: 15, color: "var(--acc2)", display: "flex", gap: 10, alignItems: "center", marginBottom: 6, animation: "slideToast .3s" }}><span style={{ flex: 1 }}>{n.msg}</span><span style={{ fontSize: 12, color: "var(--tx3)", fontFamily: "'IBM Plex Mono',monospace" }}>{n.time}</span></div>)}</div>

    {/* SIDEBAR NAV (desktop) */}
    {!isMobile && <div style={{ position: "fixed", left: 0, top: 0, bottom: 0, zIndex: 50 }}><SideNav view={view} setView={switchV} NAV={NAV} /></div>}

    {/* MAIN */}
    <div style={{ flex: 1, marginLeft: isMobile ? 0 : 200, paddingBottom: isMobile ? 80 : 20, position: "relative", zIndex: 1 }}>
      {/* TOP BAR */}
      <header className="pg" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid var(--brd-t)" }}>
        <div style={{ fontSize: 16, color: "var(--tx2)" }}>{profile.name}</div>
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={() => setTheme(t => t === "light" ? "dark" : "light")} style={{ background: "none", border: "1px solid var(--brd2)", width: 38, height: 38, cursor: "pointer", fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--tx2)" }}>{theme === "light" ? "🌙" : "☀️"}</button>
          <button onClick={() => signOut(auth)} style={{ background: "none", border: "1px solid var(--brd2)", color: "var(--tx3)", width: 38, height: 38, cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center" }}>↪</button>
        </div>
      </header>

      <main style={{ padding: 16, maxWidth: 1000, margin: "0 auto" }}>
        <div key={viewKey.current} className="vi">
          ...
          {view === "schedule" && <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 3, marginBottom: 16 }}>
              {DAYS.map((d, i) => { const it = isToday(i, wo); const hol = !!wh[i]; return <button key={d} className={it && selDay === i ? 'tp' : ''} onClick={() => goDay(i)} style={{ padding: "8px 4px", border: `1px solid ${selDay === i ? "var(--acc-brd)" : "var(--brd)"}`, background: selDay === i ? "var(--acc-dim)" : "transparent", color: selDay === i ? "var(--acc2)" : "var(--tx3)", cursor: "pointer", fontFamily: "'Barlow Condensed',sans-serif", fontSize: 14, fontWeight: 600, textTransform: "uppercase", textAlign: "center", minHeight: 52, opacity: hol ? .6 : 1 }}>
                <div>{d}</div>
                <div style={{ fontSize: 11, fontFamily: "'IBM Plex Mono',monospace", marginTop: 2, color: selDay === i ? "var(--acc2)" : "var(--tx3)" }}>{fmtDate(wd[i])}</div>
            ...
            {/* Banners */}
            {isToday(selDay, wo) && <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 14px", border: "1px solid var(--acc-brd)", background: "var(--acc-dim)", marginBottom: 12 }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--acc2)" }} /><span style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 12, color: "var(--acc2)", textTransform: "uppercase", letterSpacing: 1.5 }}>Dnes</span></div>}
            {dayHol && <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", border: "1px solid var(--acc-brd)", background: "var(--acc-dim)", marginBottom: 12 }}><span>🎉</span><span style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700, color: "var(--acc2)", textTransform: "uppercase", letterSpacing: 1, fontSize: 14 }}>{dayHol} — pracovní volno</span></div>}

            {/* Day content */}
            <div key={`${selDay}-${wo}`} className={slideDir === 'right' ? 'sr' : 'sl'}>
              {!dayHol && SHIFTS.map(shift => { const entries = dayEntries(shift); return <div key={shift} style={{ marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                  <span style={{ fontFamily: "'IBM Plex Mono',monospace", color: "var(--acc2)", fontSize: 16, fontWeight: 500 }}>{shift}</span>
                  <div style={{ flex: 1, height: 1, background: "var(--brd)" }} />
                  <span style={{ fontSize: 12, color: "var(--tx3)" }}>{entries.length} os.</span>
                </div>
                <div className="glass" style={{ padding: 0 }}>
                  {entries.map((en, idx) => { const emp = ge(en.empId); if (!emp) return null; const tc = emp.team === "L1" ? "var(--l1)" : "var(--sd)"; const noteKey = `${en.empId}-${DAYS[selDay]}-${shift}`; const note = notes[noteKey];
                    return <div key={en.empId} className="ent" draggable={canDrag(en.empId)}
                      onDragStart={e => e.dataTransfer.setData("text/plain", JSON.stringify({ empId: en.empId, day: DAYS[selDay], shift }))}
                      onClick={() => isA ? setSelCell({ day: DAYS[selDay], shift, empId: en.empId }) : en.empId === profile.id && setModal({ type: "myshift", day: DAYS[selDay], shift })}
                  ...
                  {entries.length === 0 && <div style={{ padding: "14px", color: "var(--tx3)", fontSize: 14 }}>prázdná směna</div>}
                </div>
              </div>; })}

              {/* Drop zone for D&D on this day */}
              {!dayHol && <div style={{ marginTop: 8 }}
                onDragOver={e => { e.preventDefault(); e.currentTarget.style.background = "var(--acc-dim)"; }}
                onDragLeave={e => { e.currentTarget.style.background = ""; }}
                onDrop={e => { e.currentTarget.style.background = ""; try { const d = JSON.parse(e.dataTransfer.getData("text/plain")); if (canDrop(d.empId, DAYS[selDay]) && (d.day !== DAYS[selDay] || d.shift !== SHIFTS[0])) moveE(d.empId, d.day, d.shift, DAYS[selDay], SHIFTS[0]); } catch { } }}>
              </div>}

              {/* Absences */}
              {dayAbs.length > 0 && <div style={{ marginTop: 16 }}>
                ...
                {dayAbs.map(a => { const e = ge(a.empId); const at = ABS.find(t => t.id === a.type); return e && <div key={a.empId} className="glass" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", marginBottom: 6, minHeight: 48 }}><span>{at?.icon}</span><span style={{ fontWeight: 500, color: "var(--w)", flex: 1 }}>{e.name}</span><Badge small color={at?.color || "var(--amb)"}>{at?.label}</Badge></div>; })}
              </div>}
            </div>
          </div>}
          ...
          {view === "swaps" && <div>
            ...
            {!isA && <Card style={{ marginBottom: 20 }}><p style={{ fontSize: 15, color: "var(--tx2)", marginBottom: 12 }}>Požádej o výměnu — kdokoliv může přijmout.</p><Btn warm onClick={() => setModal({ type: "swap", day: DAYS[selDay], shift: SHIFTS[0] })}>+ Nová</Btn></Card>}
            ...
            {["L1", "SD"].map(team => <div key={team} style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 12 }}>{employees.filter(e => e.team === team && e.role !== "admin").map(emp => <Card key={emp.id}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div><div style={{ fontWeight: 600, fontSize: 17, color: "var(--w)" }}>{emp.name}</div></div>
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={() => setModal({ type: "editDays", emp })} style={{ background: "none", border: "1px solid var(--brd2)", color: "var(--tx3)", cursor: "pointer", width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center" }}>✏️</button>
                    <button onClick={() => delUser(emp.id)} style={{ background: "none", border: "1px solid rgba(192,48,48,.3)", color: "var(--red)", cursor: "pointer", width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
                  </div>
                </div>
    ...
    <Modal open={!!selCell} onClose={() => setSelCell(null)} title="Akce">{selCell && (() => { const emp = ge(selCell.empId); if (!emp) return null; return <div>
      ...
      <NoteF onSubmit={n => { saveNote(selCell.empId, selCell.day, selCell.shift, n); setSelCell(null); }} />
    </div>; })()}</Modal>

    <Modal open={modal === "absence"} onClose={() => setModal(null)} title="Nepřítomnost"><AbsF emps={employees.filter(e => e.role !== "admin")} onSubmit={(e, d, t) => { addAbs(e, d, t); setModal(null); }} /></Modal>
    ...
    <Modal open={modal?.type === "myshift"} onClose={() => setModal(null)} title="Moje směna"><div>
      ...
      <NoteF onSubmit={n => { saveNote(profile.id, modal.day, modal.shift, n); setModal(null); }} />
    </div></Modal>
    <Modal open={modal === "myabsence"} onClose={() => setModal(null)} title="Nepřítomnost"><MyAbsF profile={profile} onSubmit={(d, t) => { addAbs(profile.id, d, t); setModal(null); }} /></Modal>
